...
int eval(struct node *t)
{
	while(t!=NULL)
	{
		if(t->c == '+' || t->c == '*' || t->c == '-' || t->c == '/')
		{
			switch((int)t->c - 42)
			{
				case 0: return eval(t->left) * eval(t->right);
					break;
					
				case 1: return eval(t->left) + eval(t->right);
					break;
					
				case 3: return eval(t->left) - eval(t->right);
					break;
					
				case 5: return eval(t->left) / eval(t->right);
					break;
			}
		}
